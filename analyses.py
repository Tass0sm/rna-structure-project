...
def get_stem_length(loop, label_diff):
    left_end = parse_backward_until_zero_patch(label_diff, loop[0])
    right_end = parse_until_zero_patch(label_diff, loop[1])
...
def stem_filter(n, label_diff):
    def f(loop):
        l = get_stem_length(loop, label_diff)
...
def second_automatic_loop_surrounding_analysis(labeling_df, phred_df):
    ...
    filtered_loops = list(filter(stem_filter(10), site_loops))
